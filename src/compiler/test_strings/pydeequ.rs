...
def column_level_checks_id(data_frame: DataFrame, spark_session: SparkSession) -> tuple[str, DataFrame | None]:
    try:
        ...
        data_frame_filtered = data_frame.filter(\"Price > 1\")
        check = Check(spark_session, CheckLevel.Warning,
                      \"Autogenerated check for column level rules for table Test and column Id with filter Price > 1\")
        ...
        result_df = (result_df.withColumn(\"check_category\", lit(\"column level\"))
                     ...
                     .withColumn(\"filter\", lit(\"Price > 1\")))
        data_frames.append(result_df)
        
        check = Check(spark_session, CheckLevel.Warning,
                      \"Autogenerated check for column level rules for table Test and column Id with filter \")
        check_result = VerificationSuite(spark_session).onData(data_frame).addCheck(
            check
            .isComplete(\"Id\", \"check_completeness_Test_Id\")
...
def column_level_checks_price(data_frame: DataFrame, spark_session: SparkSession) -> tuple[str, DataFrame | None]:
    try:
        ...
        data_frame_filtered = data_frame.filter(\"Price > 1\")
        check = Check(spark_session, CheckLevel.Warning,
                      \"Autogenerated check for column level rules for table Test and column Price with filter Price > 1\")
        check_result = VerificationSuite(spark_session).onData(data_frame_filtered).addCheck(
            check
            .hasDataType(\"Price\", ConstrainableDataTypes.String, lambda x: x >= 1)
        ).run()
        ...
        check = Check(spark_session, CheckLevel.Warning,
                      \"Autogenerated check for column level rules for table Test and column Price with filter \")
        check_result = VerificationSuite(spark_session).onData(data_frame).addCheck(
            check
            .satisfies(\"Price LIKE '%test%'\", \"check_like_pattern_Test_Price\", lambda x: x >= 0.5)
            .hasPattern(\"Price\", r\"[0-9]*test[0-9]*\", lambda x: x >= 0.75, \"check_has_pattern_Test_Price\")
            .satisfies(\"length(Price) > 0\", \"check_not_empty_Test_Price\", lambda x: x >= 0.75)
        ).run()
